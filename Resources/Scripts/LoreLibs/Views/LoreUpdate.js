/*
==============================================================================
//  知识编辑页面 LoreUpdate.aspx 的页面控制层代码。
==============================================================================
*/
(function ($) {
    if (!window.lorelibs) { window.lorelibs = new Object(); }
    if (!window.lorelibs.LoreUpdate_aspx) { window.lorelibs.LoreUpdate_aspx = new Object(); }
    window.lorelibs.LoreUpdate_aspx.initPage = function (ajaxContainerSelector, key) {
        window.lorelibs.lore.getLoreMasterByKey(key, function (master) {
            var nodekey = master.NodeKey;
            window.lorelibs.lorefield.getLoreNodeFieldMappingByNodeKey(nodekey, function (data) {
                window.lorelibs.lorefield.getAllLoreField(function (fielddata) {
                    ...
                    window.lorelibs.lore.getLoreByMainKey(key, function (lore) {
                        $(ajaxContainerSelector).form('loadData', lore);
                    });
                });
            });
        });
