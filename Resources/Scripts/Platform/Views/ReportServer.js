/*
==============================================================================
//  报表服务器页面 ReportServer.aspx 的页面控制层代码。
==============================================================================
*/
(function ($) {
    if (!window.platform) { window.platform = new Object(); }
    if (!window.platform.ReportServer_aspx) { window.platform.ReportServer_aspx = new Object(); }
    window.platform.ReportServer_aspx.initPage = function (ajaxContainerSelector) {
            ...
            $("#Grid", ajaxContainerSelector).datagrid({
                ...
                { text: function (e, rowIndex, rowData, eventData) {
                    ...
                    handler: function (e, rowIndex, rowData, eventData) {
                        window.platform.ReportServer_aspx.editPanel(rowData.id);
                    }
                },
                { text: function (e, rowIndex, rowData, eventData) {
                    ...
                    iconCls: "icon-no",
                    handler: function (e, rowIndex, rowData, eventData) {
                        window.platform.ReportServer_aspx.delPanel(rowData.id, rowData.title);
                    }
                }
            ],
                toolbar: [{
                    ...
                    handler: function () {
                        ...
                        if (rows.length == 0) { $.plugin.showMessage("请勾选要删除的项!"); }
                            ...
                            window.platform.ReportServer_aspx.delPanel(ids.join(','), names.join(','));
                        }
                    }
                }, '-', {
                    id: 'btnrefresh',
        ...
        window.platform.ReportServer_aspx.editPanel = function (id) {
            $.plugin.showDialog({
                title: "编辑面板",
        ...
        window.platform.ReportServer_aspx.delPanel = function (ids, names) {
