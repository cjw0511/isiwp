...
(function ($, undefined) {
    ...
    */
    function bindMenuEvent(target, menu) {
        //  注释掉本段代码，以实现当鼠标移出菜单控件时不自动隐藏，而是在菜单失去焦点后才隐藏。
    ...
    */
    ...
    $.fn.menu.parseOptions = function (target) { return $.extend({}, $.parser.parseOptions(target, ['left', 'top', { minWidth: 'number'}])); };
    ...
    var appendItemToMenu = function (menu, item, id, menus) {
        ...
        if ($.isFunction(item.onclick)) {
            itemEle.click(function (e) { item.onclick.call(this, e, item, menus); });
        }
