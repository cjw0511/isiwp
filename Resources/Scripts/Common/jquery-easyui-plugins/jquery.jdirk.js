...
(function (window, $, undefined) {
    ...
    //  定义一个空函数
    coreUtil.noop = $.isFunction($.noop) ? $.noop : function () { };
    ...
    coreArray.first = function (array, callback, thisArg) {
        array = coreArray.likeArray(array) ? array : [];
        if (!$.isFunction(callback)) { return array.length ? array[0] : null; }
        for (var i = 0; i < array.length; i++) {
    ...
    coreArray.last = function (array, callback, thisArg) {
        array = coreArray.likeArray(array) ? array : [];
        if (!$.isFunction(callback)) { return array.length ? array[array.length - 1] : null; }
    ...
    coreUtil.hasParentJquery = function (win) {
        ...
        try {
            var p = win.parent;
            ret = p && coreUtil.isWindow(p) && coreUtil.isObject(p.document) && $.isFunction(p.jQuery) ? true : false;
        } catch (ex) { }
        return ret;
    };
    ...
    coreUtil.requestAjaxBoolean = function (url, args) {
        ...
        return coreString.toBoolean(val);
    };
