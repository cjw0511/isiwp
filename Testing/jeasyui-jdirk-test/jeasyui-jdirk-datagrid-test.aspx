<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="jeasyui-jdirk-datagrid-test.aspx.cs" Inherits="ISIWP.Platform.WebClient.Testing.jeasyui_jdirk_test.jeasyui_jdirk_datagrid_test" %>

<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">

<html xmlns="http://www.w3.org/1999/xhtml">
<head runat="server">
    <title>jeasyui-jdirk-datagrid-test.aspx</title>
    ...
    <script type="text/javascript">
        $(function () {
            ...
            var columns = [
                        ...
                        { field: 'Name', title: '名称(Name)', width: 140, sortable: true, hidable: true, editor: "text" },
                        { field: 'Age', title: '年龄(Age)', width: 120, sortable: true },
                        { field: 'Height', title: '身高(Height)', width: 140, sortable: true, filter: "caps" },
                        { field: 'Weight', title: '体重(Weight)', width: 140, sortable: true, hidable: false, filter: "lower" },
                        { field: 'CreateDate', title: '创建日期(CreateDate)', width: 180, sortable: true, filter: "none" },
            ...
            var t = $("#grid").datagrid({
                ...
                columns: [columns],
                rowContextMenu: [
                    { text: "mm1", iconCls: "icon-ok", disabled: function () { return false; }, handler: function (e, rowIndex, rowData, eventData) { alert(rowData.Name); } },
                    ...
                    { text: "设置列\"名称\"的标题", iconCls: "icon-edit", handler: function () { t.datagrid("setColumnTitle", { field: "Name", title: "测试列名称" }); } },
                    { text: "设置列\"年龄\"的宽度", iconCls: "icon-edit", handler: function () { t.datagrid("setColumnWidth", { field: "Age", width: 200 }); } }
                ],
                //offset: { width: -300, height: -200 },
                selectOnRowContextMenu: true,
                hideDisabledMenu: false,
                pagination: false,
                //enableHighlightColumn: true,
                columnFilter: columnFilter//,
//                onSelect: function (index, row) { t.datagrid("beginEdit", index); },
//                onUnselect: function (index, row) { t.datagrid("endEdit", index); }
            });


            $("#Button1").click(function () { t.datagrid("setColumnFilter", columnFilter); });
            $("#Button2").click(function () { t.datagrid("setColumnFilter"); });

            $("#Button3").click(function () { t.datagrid("setColumnFilter", $.extend({}, columnFilter, { position: "top" })); });
            $("#Button4").click(function () { t.datagrid("setColumnFilter", $.extend({}, columnFilter, { position: "bottom" })); });

            $("#Button5").click(function () { t.datagrid("showRows", true); });
            $("#Button6").click(function () { t.datagrid("hideRows", true); });

            $("#Button7").click(function () {
                var fields = t.datagrid("getColumnFields"), t1 = $("#Text1"), t2 = $("#Text2"), field = t1.val(), value = t2.val();
                if (!$.array.contains(fields, field)) {
                    $.easyui.messager.show("操作提醒", "请输入当前控件中存在的 field 值", "warning", "bottomCenter");
                    t1.focus();
                    return;
                }
            ...
            $("#Button8").click(function () {
                var fields = t.datagrid("getColumnFields"), t3 = $("#Text3"), t4 = $("#Text4"), field = t3.val(), value = t4.val();
                if (!$.array.contains(fields, field)) {
                    $.easyui.messager.show("操作提醒", "请输入当前控件中存在的 field 值", "warning", "bottomCenter");
                    t3.focus();
                    return;
            ...
            $("#Button9").click(function () { t.datagrid("columnFilterSelect", { field: "Code", value: [1, 2, 3, 4, 5], selected: true }); });
            $("#Button10").click(function () { t.datagrid("columnFilterSelect", { field: "Name", value: ["汤小祥", "况文娟", "余梦恬", "陈文斌", "曾小莉"], selected: false }); });
        });
    </script>
