...
(function($) {
	...
	$.fn.jDownload = function(settings){
		...
		$(this).bind(settings.event, function(){
			...
			$.ajax({
				type : 'GET',
				url  : settings.root+'jdownload/jdownload.aspx',
					...
					setTimeout(function() {
						if(data == 'error') {
							...
							if(settings.showfileInfo == true) {
								...
								var url  = settings.root+'jdownload/jdownload.aspx';
		...
		function start_download(i) {
			...
			// change iframe src to fieDownload.aspx with filePath as query string?? 
			iframe.attr('src', settings.root+'jdownload/jdownload.aspx?action=download&path='+dialog.data('jDownloadData').filePath);
